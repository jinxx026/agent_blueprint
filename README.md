...
1. 上传文字版售后政策 PDF，系统提取文字并保存为企业知识文档。
2. 安装客服模块并生成 Blueprint，自动启用上下文切片、混合召回、rerank、压缩与引用。
3. 在线询问政策问题，查看引用链接和每个 Agent 的执行轨迹。
4. 输入“为订单 A100 创建 299 元退款”，运行时会暂停并要求主管批准，批准后才创建演示退款草稿。
5. 运行自动门禁，通过后发布测试 API；页面会显示可调用的 `POST /api/v1/apps/{blueprint_id}/invoke` 地址。

当前退款连接器是安全的演示适配器，只创建本地演示草稿，不会产生真实资金操作。接入企业 ERP/CRM 时，应由 Tool Gateway 管理凭证、网络白名单、幂等键和审计日志。
...
没有配置 Secret 时，云端默认使用内置演示后端，可以直接体验模块、知识库、Blueprint 和评测；演示数据保存在临时空间，应用重启后可能重置。

生产环境应把 FastAPI、数据库和模型网关独立部署到企业网络，再在 Streamlit 应用 Secrets 中配置：
